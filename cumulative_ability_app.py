...
@st.cache_data
def load_data():
    ca = pd.read_csv("cumulative_ability_EB_era_pos_rho_agecurve_nok.csv", low_memory=False)

    # --- NEW: check that Player_BoxScores.csv exists (for Streamlit Cloud "Files" upload) ---
    if not os.path.exists("Player_BoxScores.csv"):
        st.error(
            "Player_BoxScores.csv not found.\n\n"
            "If you're running this on Streamlit Cloud, go to your app's settings → "
            "the 'Files' section and upload **Player_BoxScores.csv** there with exactly "
            "this filename."
        )
        st.stop()
    # -----------------------------------------------------------------------

    pbs = pd.read_csv("Player_BoxScores.csv", low_memory=False)

    ca["season"] = pd.to_numeric(ca["season"], errors="coerce").astype("Int64")
    pbs["season"] = pd.to_numeric(pbs["season"], errors="coerce").astype("Int64")
    pbs["minutes_num"] = pd.to_numeric(pbs["minutes"], errors="coerce")

    # Only regular-season games where the player actually played
    mask_played = (pbs["minutes_num"] > 0) & (pbs["season_type"] == 2)
    pbs_played = pbs[mask_played].copy()
    ...
    for col in stat_cols:
        pbs_played[col] = pd.to_numeric(pbs_played[col], errors="coerce")

    agg_dict = {col: "mean" for col in stat_cols}
    agg_dict["game_id"] = "nunique"

    # Per-player-season stats (using season directly)
    pbs_season_stats = (
        pbs_played
        .groupby(["season", "athlete_id", "athlete_display_name"], as_index=False)
        .agg(agg_dict)
        .rename(columns={
    ...
    # Primary position per season
    pos_counts = (
        pbs_played
        .groupby(
            ["season", "athlete_id", "athlete_display_name",
             "athlete_position_abbreviation"],
            as_index=False
        )["game_id"]
        .nunique()
        .rename(columns={"game_id": "pos_games"})
    )

    pos_primary = (
        pos_counts
        .sort_values("pos_games", ascending=False)
        .drop_duplicates(subset=["season", "athlete_id"])
        .rename(columns={"athlete_position_abbreviation": "primary_pos"})
        [["season", "athlete_id", "primary_pos"]]
    )

    # Team games (to get primary + secondary team)
    team_games = (
        pbs_played
        .groupby(
            ["season", "athlete_id", "athlete_display_name", "team_name"],
            as_index=False
        )["game_id"]
        .nunique()
    ...
    # Remove All-Star style teams
    tg = team_games.copy()
    name = tg["team_name"].astype(str)
    allstar_mask = (
        name.str.startswith("Team ")
        | name.str.contains("All-Star", case=False, na=False)
        | name.eq("All-Stars")
    )
    team_games_clean = tg[~allstar_mask].copy()

    team_games_sorted = team_games_clean.sort_values(
        ["season", "athlete_id", "team_games"],
    ...
    primary_team = (
        ...
        [["season", "athlete_id", "team_name"]]
    )

    multi_team = team_games_sorted.groupby(
        ["season", "athlete_id"]
    ...
    if len(multi_team) > 0:
        second_team = (
            multi_team
            .groupby(["season", "athlete_id"])
            .nth(1)
            .reset_index()[["season", "athlete_id", "team_name"]]
            .rename(columns={"team_name": "team_name2"})
        )
    else:
        second_team = pd.DataFrame(columns=["season", "athlete_id", "team_name2"])

    pbs_season = (
        pbs_season_stats
        .merge(pos_primary, on=["season", "athlete_id"], how="left")
        .merge(primary_team, on=["season", "athlete_id"], how="left")
        .merge(second_team, on=["season", "athlete_id"], how="left")
    )

    merged = ca.merge(
        pbs_season[["season", "player_name", "team_name", "team_name2", "primary_pos"]],
